...
type scenario struct {
	slug  string
	build func(int, profile) ([]message, []expectedFact)
}
...
func main() {
	...
	fatal(err)
	if audit.Cases != 300 || audit.SplitCounts[benchmark.SplitTrain] != 180 || audit.SplitCounts[benchmark.SplitValidation] != 60 || audit.SplitCounts[benchmark.SplitGolden] != 60 {
	...
	files := map[benchmark.Split]string{
		benchmark.SplitTrain:      "train_v1.jsonl",
		benchmark.SplitValidation: "validation_v1.jsonl",
		benchmark.SplitGolden:     "golden_v1.jsonl",
	}
	for _, split := range []benchmark.Split{benchmark.SplitTrain, benchmark.SplitValidation, benchmark.SplitGolden} {
...
func generate() []benchmark.Case {
	...
	result := make([]benchmark.Case, 0, len(scenarios)*len(profiles))
	ordinal := 0
	for _, current := range scenarios {
		for variant, currentProfile := range profiles {
			ordinal++
			split := splitForVariant(variant)
			messages, facts := current.build(variant, currentProfile)
			result = append(result, makeCase(current.slug, variant, ordinal, split, currentProfile, messages, facts))
		}
	}
	return result
}

func makeCase(slug string, variant, ordinal int, split benchmark.Split, currentProfile profile, messages []message, facts []expectedFact) benchmark.Case {
	...
	return benchmark.Case{
		...
		ID:      fmt.Sprintf("%s-%s-%02d", slug, strings.ToLower(string(split)), variant+1),
		Split:   split,
...
func splitForVariant(variant int) benchmark.Split {
	if variant < 12 {
		return benchmark.SplitTrain
	}
	if variant < 16 {
		return benchmark.SplitValidation
	}
	return benchmark.SplitGolden
}

func scenarioCatalog() []scenario {
	return []scenario{
		{"booking-direct", func(i int, p profile) ([]message, []expectedFact) {
			...
			return []message{{"INCOMING", fmt.Sprintf(phrases[i], p.service, times[i])}}, facts(booking(0))
		}},
		{"booking-choice", func(i int, p profile) ([]message, []expectedFact) {
			...
			return []message{{"OUTGOING", fmt.Sprintf("Для услуги «%s» свободно %s.", p.service, times[i])}, {"INCOMING", answers[i]}}, facts(booking(1))
		}},
		{"booking-question", func(i int, p profile) ([]message, []expectedFact) {
			...
			return []message{{"INCOMING", fmt.Sprintf(phrases[i], times[i], p.service)}}, facts(booking(0))
		}},
		{"booking-negative", func(i int, p profile) ([]message, []expectedFact) {
			...
			return []message{{"INCOMING", formatOptional(phrases[i], times[i])}}, nil
		}},
		{"price-business", func(i int, p profile) ([]message, []expectedFact) {
			return []message{{"INCOMING", fmt.Sprintf("Сколько стоит %s?", p.service)}, {"OUTGOING", fmt.Sprintf("Стоимость услуги «%s» — %s.", p.service, amountTexts[i])}}, facts(price(1, amounts[i]))
		}},
		{"price-customer", func(i int, p profile) ([]message, []expectedFact) {
			...
			return []message{{"INCOMING", fmt.Sprintf("Интересует %s.", p.service)}, {"INCOMING", fmt.Sprintf(phrases[i], amountTexts[i])}}, facts(price(1, amounts[i]))
		}},
		{"price-no-amount", func(i int, p profile) ([]message, []expectedFact) {
			...
			return []message{{"INCOMING", fmt.Sprintf("По услуге «%s» вопрос: %s", p.service, phrases[i])}}, nil
		}},
		{"commitment-direct", func(i int, p profile) ([]message, []expectedFact) {
			...
			return []message{{"INCOMING", fmt.Sprintf("Нужны подробности про %s.", p.service)}, {"OUTGOING", promises[i]}}, facts(commitment(1))
		}},
		{"commitment-negative", func(i int, p profile) ([]message, []expectedFact) {
			...
			return []message{{"INCOMING", fmt.Sprintf("Что с запросом на %s?", p.service)}, {"OUTGOING", phrases[i]}}, nil
		}},
		{"followup-hesitation", func(i int, p profile) ([]message, []expectedFact) {
			...
			return []message{{"OUTGOING", fmt.Sprintf("Можем предложить %s.", p.service)}, {"INCOMING", phrases[i]}}, facts(followup(1))
		}},
		{"followup-refusal", func(i int, p profile) ([]message, []expectedFact) {
			...
			return []message{{"OUTGOING", fmt.Sprintf("Готовы обсудить %s.", p.service)}, {"INCOMING", phrases[i]}}, nil
		}},
		{"noise-adversarial", func(i int, p profile) ([]message, []expectedFact) {
			...
			return []message{{"INCOMING", phrases[i]}}, nil
		}},
		{"booking-and-price", func(i int, p profile) ([]message, []expectedFact) {
			return []message{{"INCOMING", fmt.Sprintf("Сколько стоит %s и можно ли записаться %s?", p.service, times[i])}, {"OUTGOING", fmt.Sprintf("Стоимость — %s, указанное время свободно.", amountTexts[i])}, {"INCOMING", "Цена подходит, запишите меня."}}, facts(price(1, amounts[i]), booking(2))
		}},
		{"booking-and-commitment", func(i int, p profile) ([]message, []expectedFact) {
			return []message{{"INCOMING", fmt.Sprintf("Хочу %s %s, проверьте возможность.", p.service, times[i])}, {"OUTGOING", "Уточню расписание специалиста и отвечу в течение часа."}, {"INCOMING", "Если окно свободно, сразу запишите меня."}}, facts(commitment(1), booking(2))
		}},
		{"price-and-followup", func(i int, p profile) ([]message, []expectedFact) {
			return []message{{"INCOMING", fmt.Sprintf("Интересует %s.", p.service)}, {"OUTGOING", fmt.Sprintf("Полная стоимость составит %s.", amountTexts[i])}, {"INCOMING", "Спасибо, мне нужно подумать; вернусь с решением завтра."}}, facts(price(1, amounts[i]), followup(2))
		}},
	}
}
