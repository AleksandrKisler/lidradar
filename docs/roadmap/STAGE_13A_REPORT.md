...
На дату первоначальной проверки 13A этап ещё не был закрыт: требовались
натурные проверки Ubuntu reboot, CUDA, загрузки модели и потери соединения.
Эти проверки позднее выполнены в части 13B; см.
...
Оставшийся натурный критерий выполнен 1 сентября 2026 года на физическом
Ubuntu-сервере с RTX 4060 через локальную Wi‑Fi-сеть. Подробные версии,
результаты перезагрузки, реального анализа и disconnect/reclaim находятся в
[`STAGE_13B_REPORT.md`](STAGE_13B_REPORT.md).
...
Вся цепочка пройдена. Для постоянной эксплуатации отдельно требуется
устойчивый HTTPS-адрес Cloud Core; это не отменяет закрытия инфраструктурного
выходного критерия этапа 13.
