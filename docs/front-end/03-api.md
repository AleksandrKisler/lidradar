...
- Все tenant-scoped операции отправляют `X-Tenant-ID: <membership.tenantId>`.
  Это относится и к Risk/Radar/SSE путям, где header пока пропущен в OpenAPI.
...
| 409 | `CONFLICT`, `EMAIL_ALREADY_REGISTERED`, `INVALID_STAGE_TRANSITION`, `IDEMPOTENCY_CONFLICT`, `RECOVERED_ALREADY_ATTRIBUTED` | понятный conflict state; refetch; не повторять автоматически |
...
| 503 | `SERVICE_NOT_READY`, `CONNECTOR_UNAVAILABLE` | сохранить безопасный draft; retry только по явному действию |
...
| `POST /locations` | T, OWNER | `CreateLocationRequest` | `201 Location` | locations |
...
Manager не может читать этот endpoint, поэтому Risk card не должна выполнять
service lookup N+1; см. [GAP-API-012](08-readiness-gaps.md#gap-api-012).
...
| `POST /integrations/{provider}/connect` | T, OWNER | `ConnectChannelRequest` | `201 ChannelConnection` | integrations, connection health |
| `DELETE /integrations/{connectionId}` | T, OWNER | — | `204` soft disconnect | integrations, health |
| `GET /integrations/{connectionId}/health` | T, OWNER | — | `200 ConnectionHealth` | — |

Connect: `400/401/403/404/503`; disconnect:
`400/401/403/404/503`; list/health: `400/401/403`, health также `404`.
`503` после disconnect означает: local state уже `DISCONNECTED`, но удаление
удалённого webhook не подтверждено; поэтому нужно refetch списка и показать
warning, а не возвращать connection визуально в active. Health — persisted
snapshot, не команда «Проверить сейчас».
...
|---|---|---|---|---|
| `GET /conversations` | T, `conversation.read` | `limit? 1..100=50`, `cursor?` | `ConversationPage` | `updatedAt` descending |
...
При смене фильтра/tenant старый cursor отбрасывается. Контракт не поддерживает
search, risk-only filter или enriched list item; browser не выполняет запрос
detail для каждой строки.

Endpoint отправки сообщений отсутствует намеренно: frontend read-only и может
лишь перейти во внешний канал после появления безопасного deeplink-контракта.
...
|---|---|---|---|---|
| `GET /radar` | T, `risks.read` | `locationId?`, `severity?`, `riskType?` | `200 RadarSummary` | — |
| `GET /risks` | T, `risks.read` | `status?`, `locationId?`, `severity?`, `riskType?`, `limit? 1..100=50`, `cursor?` | `200 {items: RiskDetail[], nextCursor}` | — |
...
должны получать одинаковые business filters. Текущий `status` допускает только
одно значение, а отсутствие значения включает terminal risks, поэтому
корректная пагинированная лента всех active statuses заблокирована
[GAP-API-003](08-readiness-gaps.md#gap-api-003).

OpenAPI сейчас пропускает tenant header у этих пяти операций; transport всё
равно обязан его отправлять. Команды над terminal risk показываются disabled
по последнему snapshot, но race окончательно разрешается backend.
...
| `POST /risks/{riskId}/actions` | T, `risks.manage`, Idempotency-Key | `{type,note?}` | `201 Action` / replay `200` | risk, risks, radar |
| `POST /opportunities/{opportunityId}/outcomes` | T, `risks.manage`, Idempotency-Key | `{status,note?}` | `201 Outcome` / replay `200` | opportunity, risk, risks, radar, analytics |
...
реальный переход во внешний канал; не записывать успех, если переход не
состоялся.
...
POST: `400/401/403/404/409`; GET: `400/401/403`. Помимо
`IDEMPOTENCY_CONFLICT`, runtime может вернуть `RECOVERED_ALREADY_ATTRIBUTED`.
...
авторитетным и используется в подписи. Макетные график и список платежей не
вычисляются из агрегатов; они ждут [GAP-API-007](08-readiness-gaps.md#gap-api-007).
...
Payload содержит только `resourceId`. На любом известном событии
инвалидируются `risk(resourceId)`, видимые `risks` и `radar`; затем REST
возвращает состояние. Unknown event игнорируется с telemetry. Сервер посылает
comment heartbeat примерно каждые 20 секунд, replay/`Last-Event-ID` нет.
...
Сигнал может быть потерян и без видимого disconnect (NOTIFY и subscriber buffer
best-effort), поэтому focus/online/manual REST refetch обязателен. Гарантия для
долго открытого видимого экрана ожидает
[GAP-RELIABILITY-020](08-readiness-gaps.md#gap-reliability-020). Runtime также
может вернуть `503 UNAVAILABLE`, если hub не инициализирован; код ещё не описан
в OpenAPI.
