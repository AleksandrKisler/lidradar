...
### Готовность API для интерфейса (закрытие разрывов фронтенда)

Аудит готовности интерфейса (`docs/front-end/08-readiness-gaps.md`) выявил
разрывы между макетами и API; они закрыты решениями ADR 0044 и ADR 0045 без
изменения границ модулей.

Модели чтения. Карточка риска (`GET /api/v1/risks`, `GET /api/v1/risks/{id}`)
содержит контакт (`id`, `displayName`), услугу сделки (`id`, `name`, `active`),
канал, `serviceId` сделки, превью последнего не удалённого сообщения (≤ 140
символов) и внешнюю ссылку `{url, kind, unavailableReason}`; все ключи
присутствуют всегда, `null` означает отсутствие связи или ещё не созданную
запись. Обогащение выполняется одним запросом для строк отобранной страницы;
ранжирование и курсор считаются только по рискам и сделкам. Список переписок
отдаёт `ConversationListItem` (контакт, канал, превью, сводка активных рисков,
внешняя ссылка) с серверными фильтрами `search`, `withRisk`, `locationId`,
`connectionId`, `status`; курсор привязан к фильтрам. Детали переписки
дополнены каналом и внешней ссылкой. Внешнюю ссылку строит только сервер по
разрешённой схеме `tg://user?id=<число>`; переход клиент фиксирует действием
`OPEN_CONVERSATION`.

Фильтры и права. Список и сводка Radar принимают повторяемый `status` (или
через запятую) и `active=true|false`; параметры взаимоисключающи, набор
статусов входит в ключ курсора. Денежные поля карточки и сводки видны любому
обладателю `risks.read`; итоги организации остаются за `revenue.read` и
`analytics.read`. Действие требует `action.manage`, исход — `outcome.manage`,
рекомендация — `risks.manage`. `Risk.source` включает `MANUAL`; пути Radar и
SSE объявляют `X-Tenant-ID`; конец страницы всегда `nextCursor: null`.

SSE. При переполнении буфера подписчика (16 сигналов) накопленные сигналы
отбрасываются и посылается одно событие `resync.required`
(`{"reason":"BUFFER_OVERFLOW"}`); клиент перечитывает сводку и открытые
списки. Без инициализированной шины — `503 UNAVAILABLE`.

Аналитика. Сводка дополнена дневным рядом `series` (по каждой дате окна в
часовом поясе организации, нули заполнены) и разбивкой `attribution`
(`RECOVERED`, `ORGANIC`, `UNKNOWN`); `GET /api/v1/analytics/payments` отдаёт
курсорный список подтверждённых событий окна во всех валютах с контактом,
услугой, атрибуцией и риском. Возвратов в модели нет.

Команда. `GET /api/v1/organization/members`, `PATCH`/`DELETE
…/members/{userId}`, `GET`/`POST /api/v1/organization/invitations`, `DELETE
…/invitations/{id}` (право `member.manage`) и `POST /api/v1/invitations/accept`
(сеанс без организации). Приглашение — одноразовый 256-битный код, показанный
один раз и хранимый как SHA-256 в таблице `membership_invitations`
(миграция `000022`), срок 7 дней; принятие создаёт членство или
восстанавливает отозванное; последний активный владелец защищён от понижения
и отзыва (`409 LAST_OWNER`). Аудит: `MEMBER_INVITED`, `INVITATION_REVOKED`,
`INVITATION_ACCEPTED`, `MEMBER_ROLE_CHANGED`, `MEMBER_REVOKED`.

Онбординг. `GET /api/v1/organization/onboarding` выводит статус из данных:
обязательные шаги — организация, активная точка с полным недельным графиком,
активная услуга, подключение не в `DISCONNECTED`; личная привязка Telegram
необязательна. Сервер не хранит «пройденные шаги».

Подключение каналов. Секрет webhook может выпустить сервер (256 бит,
возвращается один раз, для Telegram не возвращается); одноразовая передача
токена бота из браузера по TLS принята для MVP (шифрование с привязкой к
подключению, токен не возвращается и не логируется);
`POST /api/v1/integrations/{id}/health/check` выполняет живую проверку
Telegram (`getWebhookInfo`) и сохраняет результат (`verification: REMOTE`),
для остальных провайдеров возвращает сохранённое состояние (`LOCAL`). Поле
`active` при создании точки отклоняется `400`.
